...
class VisionGridClassifier:
    ...
    def _classify_reference_local(self, reference_source: str, source_list: List[str]) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                # Dynamic relative margin: only tiles whose reference-category
                # logit is clearly better (higher, less negative) than the mean
                # of all tiles count as a match.
                selected = self._select_reference_matches(scores)
    ...
    def _target_text(instruction: str) -> str:
        ...
        patterns = [
            ...
            r"(?i)^.*?(?:wähle|wählen|klicke|anklicken|markiere|markieren|finde|finden|suche|suchen)\s+(?:sie\s+)?(?:alle\s+)?(?:bilder|felder|kacheln|tiere|fotos)?\s*(?:mit|von|auf denen|nach|die)\s+",
            r"(?i)^.*?(?:select|click|choose|mark|find)\s+(?:all\s+)?",
            r"(?i)^.*?(?:wähle|wählen|klicke|anklicken|markiere|markieren|finde|finden|suche|suchen)\s+(?:sie\s+)?(?:alle\s+)?(?:auf\s+)?",
        ]
    ...
    def _select_top_k(
        ...
        cycle. Always take the top-k highest-scoring tiles and let the existing
        submit+verify retry loop handle a wrong answer (a new grid costs another
        round, never a ban)."""
        ranked = sorted(
    ...
    def _select_reference_matches(self, scores: List[float | None]) -> List[int]:
        ...
        A tile is a match only when its reference score is clearly better
        (higher, i.e. less negative) than the mean of all tiles, measured in
        standard deviations. Replaces "any tile whose argmax happened to equal
        the reference category" (which selected almost the whole grid on noisy
        scores) without a fixed Top-k."""
        ...
        std = variance ** 0.5
        threshold = mean + self.reference_margin * std
        selected = [
            index for index, score in enumerate(scores)
            if score is not None and float(score) >= threshold
        ]
        if not selected:
            # No clear match: leave empty so the caller requests a fresh
            # challenge (reload) instead of guessing blindly.
            return []
        if len(selected) < 3:
            # reCAPTCHA almost never accepts fewer than 3 tiles. If the relative
            # filter found only 1-2 clear matches, add the next-best unselected
            # tiles (by logit score) until exactly 3 are selected.
            remaining = sorted(
                (
                    index for index, score in enumerate(scores)
                    if score is not None and index not in selected
                ),
    ...
    @staticmethod
    def _reference_margin(value: str) -> float:
        try:
